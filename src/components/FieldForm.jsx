...
export default function FieldForm({ farmerId, onAdded }) {
  ...
  async function handleLocateAddress() {
    ...
    try {
      ...
      setError(e.message || "Konum bulunamadı")
    } finally {
      setLocating(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="field-form">
      <div className="field-form__row">
        <input
          placeholder="Tür (örn. üzüm, zeytin)"
          value={type}
          onChange={(event) => {
        ...
        <button type="button" onClick={handleLocateAddress} disabled={locating} className="btn btn--secondary">
          {locating ? "Konum aranıyor..." : "Adrese git"}
        </button>
      </div>
      <Suspense fallback={<div className="field-form__map field-form__map-loading">Harita yükleniyor...</div>}>
        <LocationPicker
