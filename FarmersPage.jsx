...
export default function FarmersPage() {
    ...
    try {
      ...
      setErr(
        e.code === "permission-denied"
          ? "Hesabiniz yetkili degil. Lutfen yoneticinize bildiriniz."
          : e.message || "Beklenmedik bir hata olustu."
      )
    } finally {
      setLoading(false)
    }
  ...
  async function handleSaveEdit(e) {
    ...
    if (!trimmedName) {
      setEditErr("Isim alani zorunludur.")
      return
    }
    setEditBusy(true)
    setEditErr("")
    try {
      ...
      setEditErr(error?.message || "Guncelleme basarisiz.")
    } finally {
      setEditBusy(false)
    }
  }
  ...
  async function handleDelete(farmer) {
    ...
    try {
      ...
      if (hasFields) {
        alert("Once bu ciftciye ait alanlari silin. (Alt koleksiyonlar otomatik silinmez.)")
        return
      }
  ...
  if (loading) return <div className="app-loading">Yukleniyor...</div>
  ...
  return (
    <div className="farmers">
      ...
      {farmers.length === 0 ? (
        <div className="farmers__empty">Henuz kayit yok. Yukaridan yeni ciftci ekleyebilirsiniz.</div>
      ) : (
          ...
          {farmers.map((farmer) => {
            ...
            return (
              <div
                ...
                className={`farmers__card ${expanded ? "farmers__card--expanded" : ""}`.trim()}
              >
                <div className="farmers__card-header">
                  <div className="farmers__card-title">
                    <span className="farmers__card-name">{farmer.name || "Isim bilinmiyor"}</span>
                    <span className="farmers__card-phone">{farmer.phone || "Telefon bilgisi yok"}</span>
                  </div>
                  <button
                    type="button"
                    className={`farmers__toggle ${expanded ? "farmers__toggle--expanded" : ""}`.trim()}
                    onClick={(event) => {
                ...
                {expanded && (
                  <div className="farmers__details">
                    ...
                    {isEditing ? (
                      ...
                      >
                        <div className="farmers__editor-title">Ciftci bilgilerini duzenle</div>
                        <input
                          value={editName}
                        ...
                        <input
                          ...
                          placeholder="Telefon"
                        />
                        {editErr && <div className="farmers__error text-small">{editErr}</div>}
